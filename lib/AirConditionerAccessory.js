...
class AirConditionerAccessory extends BaseAccessory {
	...
	constructor(...props) {
		...
		this.cmdCool = this.constants.COOL_CMD;
		...
		this.cmdHeat = this.constants.HEAT_CMD;
		...
		this.cmdAuto = this.constants.AUTO_CMD;
		...
		// Fan command defaults can be model-specific
		this.cmdFan = this.constants.CMD_FAN || 'fan';
		if (this.device.context.cmdFan) this.cmdFan = ('' + this.device.context.cmdFan).trim();

		// Dry command defaults can be model-specific
		this.cmdDry = this.constants.CMD_DRY || 'dehum';
		if (this.device.context.cmdDry) this.cmdDry = ('' + this.device.context.cmdDry).trim();

		// hard coded for tadiran
		this._rotationSteps = this.constants.FAN_SPEEDS;
		this.device.context.noSwing = true;

		this.initializeTemperatureValues();
	}

	initializeTemperatureValues() {

		const dps = this.device.state;	
		const reportedTemp = dps['2'] / this.constants.DP2_SET_TEMP_FACTOR;
		console.log("[TadiranAC] Initial DPS temperature:", dps['2'], "=>", reportedTemp);
	
		if (!this.device.context.noCool && this.characteristicCoolingThresholdTemperature)
			this.characteristicCoolingThresholdTemperature.updateValue(reportedTemp);
	
		if (!this.device.context.noHeat && this.characteristicHeatingThresholdTemperature)
			this.characteristicHeatingThresholdTemperature.updateValue(reportedTemp);
	}
	...
	_registerCharacteristics(dps) {
		...
		const characteristicCurrentTemperature = service.getCharacteristic(Characteristic.CurrentTemperature)
			.updateValue(dps['3'] / this.constants.DP3_ROOM_TEMP_FACTOR)
		...
		if (!this.device.context.noCool) {
			characteristicCoolingThresholdTemperature = service.getCharacteristic(Characteristic.CoolingThresholdTemperature)
				...
				.updateValue(dps['2'] / this.constants.DP2_SET_TEMP_FACTOR)
		...
		this.device.on('change', (changes, state) => {
			...
			if ('2' in changes) {
				const reportedTemp = changes['2'] / this.constants.DP2_SET_TEMP_FACTOR;
			
				if (!this.device.context.noCool && characteristicCoolingThresholdTemperature && characteristicCoolingThresholdTemperature.value !== reportedTemp)
					characteristicCoolingThresholdTemperature.updateValue(reportedTemp);
			
				if (!this.device.context.noHeat && characteristicHeatingThresholdTemperature && characteristicHeatingThresholdTemperature.value !== reportedTemp)
					characteristicHeatingThresholdTemperature.updateValue(reportedTemp);
			}

			if ('3' in changes && characteristicCurrentTemperature.value !== changes['3']) characteristicCurrentTemperature.updateValue(changes['3'] / this.constants.DP3_ROOM_TEMP_FACTOR);
	...
	getState(dp, callback) {
		if (dp === '2' || dp === 2) {
			const raw = this.device.state[dp];
			const scaled = raw / this.constants.DP2_SET_TEMP_FACTOR;
			console.log(`[AirConditioner] getState dp=${dp}, raw=${raw}, scaled=${scaled}`);
			return callback(null, scaled);
		}
	
		// Fall back to base method
		super.getState(dp, callback);
	}
	...
	_getCurrentTemperature(dp) {
		return (dp / this.constants.DP3_ROOM_TEMP_FACTOR);
	}
	...
	_getCurrentHeaterCoolerState(dps) {
		...
		switch (dps['4']) {
			...
			case this.cmdAuto:
				this.accessory.context.lastAcMode = this.cmdAuto
				if ((dps['3'] / this.constants.DP3_ROOM_TEMP_FACTOR) < dps['2'] / this.constants.DP2_SET_TEMP_FACTOR)
					return Characteristic.CurrentHeaterCoolerState.HEATING;
				else
					return Characteristic.CurrentHeaterCoolerState.COOLING;
	...
	setTargetThresholdTemperature(mode, value, callback) {
		...
		const scaledValue = Math.round(value * this.constants.DP2_SET_TEMP_FACTOR);

		this.setState('2', scaledValue, err => {
			if (err) return callback(err);
	...
	_getRotationSpeed(dps) {
		...
		if (this.constants.SILENT_FAN_MODE != '0' && dps[this.constants.SILENT_FAN_MODE]) {
			// With 'mute'/'sleep' removed, treat Silent as the lowest available step
			return this._hkRotationSpeed = Math.round(100 * (1) / this._rotationSteps.length);
		}

		if (this.constants.TURBO_FAN_MODE != '0' && dps[this.constants.TURBO_FAN_MODE]) {
			return this._hkRotationSpeed = 100;
		}
	...
	setRotationSpeed(value, callback) {
		...
		if (this.constants.SILENT_FAN_MODE !== '0' && value < Math.round(100 / (this._rotationSteps.length + 2))) {
			dpsPayload[this.constants.SILENT_FAN_MODE] = true;
			dpsPayload[this.constants.TURBO_FAN_MODE] = false;
			dpsPayload['5'] = this._rotationSteps[0];
		} else if (this.constants.TURBO_FAN_MODE !== '0' && value === 100) {
			dpsPayload[this.constants.SILENT_FAN_MODE] = false;
			dpsPayload[this.constants.TURBO_FAN_MODE] = true;
		} else {
			dpsPayload['5'] = this.convertRotationSpeedFromHomeKitToTuya(value);
		
			if (this.constants.SILENT_FAN_MODE !== '0'){
				dpsPayload[this.constants.SILENT_FAN_MODE] = false;
			}
			if (this.constants.TURBO_FAN_MODE !== '0') {
				dpsPayload[this.constants.TURBO_FAN_MODE] = false;
			}
		}

		this.setMultiState(dpsPayload, callback);
	}
	...
	convertRotationSpeedFromTuyaToHomeKit(value) {
		// If value is unknown (e.g., 'auto' reported by device but not in steps),
		// map it to the highest known level instead of 0%
		const index = this._rotationSteps.indexOf(value);
		const effectiveIndex = index === -1 ? (this._rotationSteps.length - 1) : index;
		return Math.round(100 * (effectiveIndex + 1) / this._rotationSteps.length)
