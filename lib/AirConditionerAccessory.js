...
class AirConditionerAccessory extends BaseAccessory {
	...
	_registerCharacteristics(dps) {
		...
		if (!this.device.context.noCool) {
			...
			const reportedTemp = dps['2'] / 10;

			console.error("***************************** _registerCharacteristics:: UPDATING HK COOL STATE", reportedTemp);
			characteristicCoolingThresholdTemperature = service.getCharacteristic(Characteristic.CoolingThresholdTemperature)
				...
				.updateValue(reportedTemp)
				.on('get', this.getState.bind(this, '2'))
				.on('set', this.setTargetThresholdTemperature.bind(this, 'cold'));
		...
		if (!this.device.context.noHeat) {
			...
			console.error("***************************** _registerCharacteristics:: UPDATING HK HEAT STATE", reportedTemp);
			...
			characteristicHeatingThresholdTemperature = service.getCharacteristic(Characteristic.HeatingThresholdTemperature)
				...
				.updateValue(reportedTemp)
				.on('get', this.getState.bind(this, '2'))
				.on('set', this.setTargetThresholdTemperature.bind(this, 'hot'));
		...
		console.error("*******on change triggered!!!!");
			...
			if ('2' in changes) {
				const reportedTemp = changes['2'] / 10;
				...
				if (!this.device.context.noCool && characteristicCoolingThresholdTemperature && characteristicCoolingThresholdTemperature.value !== reportedTemp)
					...
					console.error("***************************** onChange:: UPDATING HK COLLING STATE", reportedTemp);
				...
				if (!this.device.context.noHeat && characteristicHeatingThresholdTemperature && characteristicHeatingThresholdTemperature.value !== reportedTemp)
					console.error("***************************** onChange:: UPDATING HK HEATING STATE", reportedTemp);
	...
	initializeTemperatureValues() {
		...
		console.error("***************************** initializeTemperatureValues");

		const dps = this.device.state;


		console.error("*****************************DPS 2 IS ", dps['2']);
		...
		if (!this.device.context.noCool && this.characteristicCoolingThresholdTemperature)
			console.error("***************************** initializeTemperatureValues:: UPDATING HK COLLING STATE", reportedTemp);
		...
		if (!this.device.context.noHeat && this.characteristicHeatingThresholdTemperature)
			console.error("***************************** initializeTemperatureValues:: UPDATING HK HEATING STATE", reportedTemp);
	...
	getCurrentTemperature(callback) {
		this.getState(['3', '130'], (err, dps) => {
		  if (err) {
			console.error('[ERROR] getState failed:', err);
			return callback(err);
		  }
	  
		  console.error('[DEBUG] DPS values:', JSON.stringify(dps));
	  
		  if (dps['3'] !== undefined) {
			const celsius = dps['3'];
			console.error('[DEBUG] Using DP 3 value:', celsius);
			callback(null, celsius);
		  } else {
			console.error('[WARN] No temperature data in DPS');
			callback(null, 0);
		  }
		});
	  }
	...
	setTargetThresholdTemperature(mode, value, callback) {
		const scaledValue = Math.round(value * 10); // this goes to the AC
		this.setState('2', scaledValue, err => {
			if (err) return callback(err);
	
			if (mode === 'cold' && !this.device.context.noCool && this.characteristicCoolingThresholdTemperature) {
				console.error("***************************** setTargetThresholdTemperature:: UPDATING HK COLLING STATE", value);
				this.characteristicCoolingThresholdTemperature.updateValue(value);
			  } else if (mode === 'hot' && !this.device.context.noHeat && this.characteristicHeatingThresholdTemperature) {
				console.error("***************************** setTargetThresholdTemperature:: UPDATING HK HEATING STATE", value);
